...
MStatus harmonics::compute( const MPlug& plug, MDataBlock& data ){
    ...
    if( plug == aStorage ) {
		...
		if (pd != nullptr) {
			...
			if (!accel.empty()) {
				Vec3 ret = harmonicSolver(frame, waves, length, ampl, decay, ampAxis, !normAmp, accel, ignoreFirst);
...
MStatus harmonics::setDependentsDirty(const MPlug& plug, MPlugArray& plugArray) {
	...
	if (upPlug.asBool()){
		if (plug == aStorage){
			plugArray.append(MPlug(thisMObject(), aAccel));
		}
		...
		if ( plug == aStep || plug == aTime || plug == aUpdate ||
			...
			plugArray.append(MPlug(thisMObject(), aAccel));
			plugArray.append(MPlug(thisMObject(), aStorage));
		}
	}
